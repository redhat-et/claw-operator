...
- `Claw` — Main CRD. Spec: `config` (ConfigSpec: raw RawExtension, mergeMode merge/overwrite), `credentials` ([]CredentialSpec), `auth` (AuthSpec: mode token/password, passwordSecretRef, disableDevicePairing), `version` (string, overrides OpenClaw image tag per-instance). Status: `conditions`, `url`, `gatewayTokenSecretRef`
