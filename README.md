...
- **Network isolation** -- OpenClaw pods reach external APIs through the credential proxy by default. Messaging channels that open WebSocket sessions currently get direct TCP/443 gateway egress until those channel clients can be proxy-routed. The proxy only allows HTTPS (port 443) egress and rejects any domain not explicitly configured.
