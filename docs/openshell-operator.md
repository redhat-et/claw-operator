...
spec:
  image: quay.io/<org>/openclaw:openshell
  credentials:
    - name: openai
      provider: openai
      type: bearer
...
7. uses `spec.image` for the OpenClaw gateway and init images
...
If agent exec reports `spawn /opt/openshell/bin/openshell ENOENT`, the image
selected by `spec.image` does not contain the OpenShell CLI at the expected path.
