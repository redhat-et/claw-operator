...
func TestInjectMetricsConfig(t *testing.T) {
	t.Run("should inject diagnostics.otel.metrics when metrics enabled", func(t *testing.T) {
		...
		// endpoint is not set by injectMetricsConfig; injectDiagnosticsConfig owns it
		_, hasEndpoint := otel["endpoint"]
		assert.False(t, hasEndpoint)
	})

	t.Run("should preserve existing diagnostics.otel keys", func(t *testing.T) {
		...
		assert.Equal(t, "http://collector.svc:4318", otel["endpoint"], "endpoint should be preserved")
		assert.Equal(t, true, otel["traces"], "traces should be preserved")
	...
	t.Run("should not override user-set metrics false", func(t *testing.T) {
		...
		otel := config["diagnostics"].(map[string]any)["otel"].(map[string]any)
		assert.Equal(t, false, otel["metrics"], "user metrics:false should be respected")
	})
...
func TestMetricsEnrichmentPipeline(t *testing.T) {
	t.Run("should enrich service and NP when metrics enabled", func(t *testing.T) {
		svc := makeTestServiceForMetrics()
		np := makeTestNetworkPolicyForMetrics()

		objects := []*unstructured.Unstructured{svc, np}
		instance := testClawWithMetrics(true, nil)
...
func TestInjectMetricsConfigJSONRoundTrip(t *testing.T) {
	...
	t.Run("should preserve user diagnostics.otel when merging", func(t *testing.T) {
		operatorJSON := `{
			"gateway": {"port": 18789},
			"diagnostics": {"otel": {"enabled": true, "endpoint": "http://collector:4318", "traces": true}}
		}`
		...
		otel := roundTripped["diagnostics"].(map[string]any)["otel"].(map[string]any)
		assert.Equal(t, "http://collector:4318", otel["endpoint"], "user endpoint preserved")
...
func TestMetricsIntegration(t *testing.T) {
	...
	t.Run("should add metrics port/NP and operator.json metrics flag after reconciliation", func(t *testing.T) {
		...
		// Verify Deployment does NOT have an OTel collector sidecar
		...
		for _, c := range deployment.Spec.Template.Spec.Containers {
			assert.NotEqual(t, "otel-collector", c.Name, "no collector sidecar expected")
		}

		// Verify ConfigMap does NOT have otel-collector.yaml (no sidecar)
		cm := &corev1.ConfigMap{}
		waitFor(t, timeout, interval, func() bool {
			return k8sClient.Get(ctx, client.ObjectKey{
				Name:      getConfigMapName(testInstanceName),
		...
		_, hasCollectorYAML := cm.Data["otel-collector.yaml"]
		assert.False(t, hasCollectorYAML, "ConfigMap should not have otel-collector.yaml")
	...
	t.Run("should not inject metrics when disabled", func(t *testing.T) {
		...
		for _, p := range svc.Spec.Ports {
			assert.NotEqual(t, "metrics", p.Name, "no metrics port when disabled")
		}
	})
	...
	t.Run("should use custom metrics port on Service", func(t *testing.T) {
		t.Cleanup(func() { deleteAndWaitAllResources(t, namespace) })
